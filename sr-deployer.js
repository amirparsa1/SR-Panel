	...
	async fetch(request, env, ctx) {
		...
		if (request.method === "POST" && url.pathname === "/api/list-panels") {
			try {
				...
				try {
					const ghRes = await fetch("https://raw.githubusercontent.com/amirparsa1/SR-Panel/main/zeus.js?t=" + Date.now());
					if (ghRes.ok) {
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex flex-col items-center justify-center p-4">
    <div id="mainCard" class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-3xl shadow-2xl p-8 relative overflow-hidden z-10">
        ...
        <div class="absolute -right-12 -bottom-12 w-40 h-40 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none"></div>
            ...
            <p class="text-sm font-medium text-gray-500 dark:text-zinc-400">نصب خودکار پنل SR روی کلودفلر</p>
...
<div class="flex flex-col gap-4 mt-6 z-10">
    <div class="flex items-center gap-4 justify-center">
        <a href="https://github.com/amirparsa1/SR-Panel" target="_blank" class="flex items-center gap-2 px-4 py-2 border border-gray-700 text-gray-500 bg-gray-900/20 hover:bg-gray-900/40 rounded-full shadow-sm hover:shadow-md transition text-sm font-bold group">
    ...
    <div class="flex items-center gap-4 justify-center">
        <a href="https://sr-deployer.ir-srroot.workers.dev/" target="_blank" class="flex items-center gap-2 px-4 py-2 border border-amber-700 text-amber-500 bg-amber-900/20 hover:bg-amber-900/40 rounded-full shadow-sm hover:shadow-md transition text-sm font-bold group">
