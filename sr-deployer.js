		...
		if (request.method === "POST" && url.pathname === "/api/list-panels") {
			try {
				...
				for (let script of scriptsData.result) {
					if (script.id.startsWith("SR-panel") || script.id.startsWith("ez-")) {
...
<div class="flex flex-col gap-4 mt-6 z-10">
    ...
    <div class="flex items-center gap-4 justify-center">
        <a href="#" target="_blank" class="flex items-center gap-2 px-4 py-2 border border-amber-700 text-amber-500 bg-amber-900/20 hover:bg-amber-900/40 rounded-full shadow-sm hover:shadow-md transition text-sm font-bold group">
    ...
    try {
        ...
        async function startDeploy() {
            ...
            try {
                ...
                statusText.innerText = 'در حال دریافت پنل SR پنل...';
